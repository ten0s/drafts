...
// From https://github.com/romgrk/node-gtk/blob/master/src/debug.cc

#include "debug.h"

gchar *GetTypeName (GITypeInfo *type_info) {
    ...
    switch (type_tag) {
        ...
        case GI_TYPE_TAG_GSLIST:
        {
            ...
            gchar *elem_name = GetTypeName (elem_info);
            gchar *result = g_strdup_printf("%s[]", elem_name);
...
void print_callable_info (GICallableInfo *info) {
    ...
    gchar *typeName = GetTypeName(&return_info);
    ...
    for (int i = 0; i < n_args; i++) {
        ...
        gchar *typeName = GetTypeName(&type_info);
